...
export default async function IncomingInvoiceDetailPage({
  ...
  if (!invoice || invoice.type !== 'INCOMING') notFound()
        ...
        <div className="flex items-center gap-2">
          ...
          {invoice.status === 'DRAFT' && (
            <Button variant="outline" size="sm" asChild>
              <Link href={`/dashboard/invoices/incoming/${invoice.id}/edit`}>
                <Pencil className="mr-2 h-4 w-4" />
                Edit
              </Link>
            </Button>
          )}
          <DeleteInvoiceButton id={invoice.id} status={invoice.status as InvoiceStatus} type="INCOMING" redirectAfter />
        </div>
      </div>
        ...
        <div className="space-y-4">
          ...
          <Card>
            <CardContent className="pt-6">
              <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground mb-3">Amount</p>
